...
class Solution:
    def cheapestJump(self, A, B):
        ...
        self.paths = []
        self.findPaths(dp, A, B, len(A)-1, [])
        if len(self.paths) == 0:
            return []
        return min(self.paths)

    def findPaths(self, dp, A, B, pos, cur):
        if pos == 0:
            self.paths.append(tuple([1]+cur))
        else:
            minn = dp[pos] - A[pos]
            for i in range(pos-1, pos-B-1, -1):
                if i >= 0 and dp[i] == minn:
                    self.findPaths(dp, A, B, i, [pos+1] + cur)
