    ...
    def fallingSquares(self, positions):
        ...
        for pos, height in positions:
            ...
            for inter in intervals:
                if inter[0] >= pos+height or inter[1] <= pos:
                    continue
                base = max(base, inter[2])
            if len(res) == 0:
                res.append(base+height)
            else:
                res.append(max(res[-1], base+height))
            intervals.append((pos, pos+height, base+height))
        return res
