...
class Solution:
    class MyHeap:
        ...
        def add(self, x):
            ...
            if x not in self.dic:
                ...
                self.dic[x].append(data)
            self.heap.append(data)
            i = len(self.heap)
            while i != 1 and self.heap[(i>>1)-1][0] < x:
                self.heap[(i>>1)-1], self.heap[i-1] =\
                    self.heap[i-1], self.heap[(i>>1)-1]
                i >>= 1

        def remove(self, x):
            self.dic[x].pop()[1] = False
        ...
        def _pop(self):
            ...
            self.heap.pop()
            i = 1
            while (i<<1)-1 < len(self.heap):
                nxt = (i<<1)-1
                if i*2 < len(self.heap):
                    if self.heap[(i<<1)-1][0] < self.heap[i<<1][0]:
                        nxt = i<<1
                if self.heap[nxt][0] > self.heap[i-1][0]:
                    self.heap[nxt], self.heap[i-1] =\
                        self.heap[i-1], self.heap[nxt]
                    i = nxt+1
                else:
                    break
    ...
    def getSkyline(self, buildings):
        ...
        """
        # O(nlgn) using heap
        # Construct [L, R, H] to [(L, H, -1), (R, H, 1)] vertical lines
        lines = []
        for b in buildings:
            lines.append((b[0], b[2], -1))
            lines.append((b[1], b[2], 1))
        lines = sorted(lines, key=lambda x: (x[0], x[1]*x[2]))
        heap, res = self.MyHeap(), []
        for x, h, t in lines:
            if t == -1:
                if h > heap.max():
                    res.append([x, h])
