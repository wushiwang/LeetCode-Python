...
class Excel:
    def __init__(self, H, W):
        ...
        :type W: str
        """
        self.excel = [[0]*(ord(W)-ord('A')+1) for _ in range(H+1)]
        self.functable = dict()
    ...
    def set(self, r, c, v):
        ...
        :rtype: void
        """
        if (r, c) in self.functable:
            self.functable.pop((r, c))
        self.excel[r][ord(c)-ord('A')] = v
    ...
    def get(self, r, c):
        ...
        :rtype: int
        """
        if (r, c) not in self.functable:
            return self.excel[r][ord(c)-ord('A')]
        else:
            return self.sum(r, c, self.functable[(r, c)])
    ...
    def sum(self, r, c, strs):
        ...
        for str in strs:
            if ':' not in str:
                rr, cc = int(str[1:]), str[0]
                s += self.get(rr, cc)
            else:
                pos = str.find(':')
                lr, lc = int(str[1:pos]), str[0]
                rr, rc = int(str[pos+2:]), str[pos+1]
                for i in range(lr, rr+1):
                    for j in range(ord(lc)-ord('A'), ord(rc)-ord('A')+1):
                        s += self.get(i, chr(ord('A')+j))
        self.excel[r][ord(c)-ord('A')] = s
        self.functable[(r, c)] = strs
        return s
