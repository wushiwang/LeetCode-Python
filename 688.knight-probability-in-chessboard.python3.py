...
class Solution:
    def knightProbability(self, N, K, r, c):
        ...
        :rtype: float
        """
        self.dp = [[[None]*(K+1) for _ in range(N)] for _ in range(N)]
        self.dx = [1, 2, 1, 2, -1, -2, -1, -2]
        self.dy = [-2, -1, 2, 1, -2, -1, 2, 1]
        return self.DFS(r, c, K, N)

    def DFS(self, x, y, k, N):
        if k == 0:
            return 1
        if self.dp[x][y][k] is not None:
            return self.dp[x][y][k]
        res = 0
        for d in range(8):
            nx, ny = x+self.dx[d], y+self.dy[d]
            if nx >= 0 and nx < N and ny >= 0 and ny < N:
                res += (1/8)*(self.DFS(nx, ny, k-1, N))
        self.dp[x][y][k] = res
        return res
