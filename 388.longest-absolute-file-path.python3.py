    ...
    def lengthLongestPath(self, input):
        ...
        while i < len(input):
            ...
            if j != 0:
                ...
                cur = input[i].lstrip('\t')
                tag = len(input[i]) - len(cur)
            if len(stack) != 0 and tag > stack[-1][2] + 1:
                cur = ' '*(tag-stack[-1][2]-1)*4 + cur
                tag = stack[-1][2] + 1
            while len(stack) != 0 and tag <= stack[-1][2]:
                stack.pop()
            if '.' in cur:
                if len(stack) != 0:
                    res = max(res, len(cur)+stack[-1][1]+tag)
                else:
                    res = max(res, len(cur))
            else:
                if len(stack) != 0:
                    stack.append((cur, len(cur)+stack[-1][1], tag))
                else:
                    stack.append((cur, len(cur), tag))
            i += 1
        return res
