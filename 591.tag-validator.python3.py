...
class Solution:
    def isValid(self, code):
        ...
        while pos < len(code):
            if code[pos] == '<':
                tocken = self.getTocken(code, pos)
                t, v = self.tockenType(tocken)
                if t == 3:
                    return False
                elif t == 1:
                    if (not self.checkTagName(v)) or\
                            len(stack) == 0 or stack[-1] != v:
                        return False
                    stack.pop()
                elif t == 2:
                    if not self.checkTagName(v):
                        return False
                    if len(stack) == 0:
                        snap += 1
                    stack.append(v)
                ...
                if len(stack) == 0:
                    return False
                pos += 1
        return len(stack) == 0 and snap == 1

    def getTocken(self, code, pos):
        if pos+1 < len(code) and code[pos+1] == '!':
            res = code[pos:].find(']]>')
            if res != -1:
                return code[pos:][:res+3]
            else:
                return code[pos:]
        else:
            res = code[pos:].find('>')
            if res != -1:
                return code[pos:][:res+1]
            else:
                return code[pos:]

    def tockenType(self, tocken):
        if tocken[:9] == '<![CDATA[' and tocken[-3:] == ']]>':
            return 0, None
        elif tocken[:2] == '</' and tocken[-1] == '>':
            return 1, tocken[2:-1]
        elif tocken[0] == '<' and tocken[-1] == '>':
            return 2, tocken[1:-1]
        else:
            return 3, None

    def checkTagName(self, name):
